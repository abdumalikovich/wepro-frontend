  ...
  <div class="widget">
      ...
      >
        ...
        <div class="wrapper menu-links">
          <a
            @click="СloseForms()"
            target="_blank"
            href="https://madad.studio"
            >{{ $t("links.madad") }}</a
          >
          <a @click="СloseForms()" href="/#courses">{{
            $t("links.courses")
          }}</a>
          <a @click="СloseForms()" href="/#reviews">{{ $t("links.video") }}</a>
          <a @click="СloseForms(), OpenForm(['Application'])" href="#">{{
            $t("links.video")
          }}</a>
          <a
            @click="СloseForms()"
            target="_blank"
            href="https://t.me/weprouz_bot"
            >{{ $t("links.telegram") }}</a
          >
        ...
        <div class="footer">
          <div class="side elements-pc side-left bolder menu-footer">
            <div class="side-left-left">
              <span>© 2021 Wepro. Все права защищены.</span>
            </div>
            <div class="side-left-right">
              <span
                ><a href="tel:+998 90 197 71-00">+998 90 197 71-00</a> |
                <a href="mailto:hello@wepro.uz">hello@wepro.uz</a></span
              >
            </div>
          </div>
          <div class="side right-side t-20 menu-social">
            <a target="_blank" href="https://www.instagram.com/wepro.uz/"
              >Instagram</a
            >
            ...
            <a
              target="_blank"
              href="https://www.youtube.com/channel/UCF2_hjZ9xENm01x4gPoLakg"
              >YouTube</a
            >
            <a target="_blank" href="https://www.facebook.com/wepro.uz"
              >Facebook</a
            >
            <!-- <a href="#">Behance</a> -->
          </div>
    ...
    <transition name="bottom-top">
      <div class="widget-def widget-video" v-if="Form.Video">
        <iframe
          src="https://www.youtube.com/embed/sffj_Y1arJI?autoplay=1&mute=1&loop=1"
    ...
    <transition name="bottom-top">
      <div class="widget-def widget-success" v-if="Form.Success">
        ...
        <div class="heading padding-sides">
          <span class="h4"
            >Спасибо. Мы получили заявку и ответим тебе в течение дня!</span
          >
        </div>
        <div class="wrapper padding-sides">
          ...
          <ul>
            <li class="note">
              Мы позвоним тебе и уточним когда можно будет подойти в центр
            </li>
            <li class="note">Ответим на все имеющиеся у тебя вопросы</li>
            <li class="note">Поговорим о профессии, которая тебя интересует</li>
            <li class="note">
              При необходимости - подберем удобный график обучения
            </li>
          </ul>
          <p class="note t-black">
            Обычно, мы отвечаем в течение дня. До встречи у нас, твой «Wepro» :)
          </p>
      ...
      <div class="widget-def widget-success" v-if="Error.Status">
        ...
        <div class="wrapper padding-sides">
          <a href="tel:+998 90 197 71-00">
            <p class="t-20">
              Пожалуйста, попробуйте еще раз или перезвоните по номеру
              <b class="blue">+998 90 197 71-00</b>
            </p>
          </a>
    ...
    <transition name="bottom-top">
      <div class="widget-def widget-application" v-if="Form.Application">
        ...
        <form
          class="vertical padding-sides"
          @submit="Add({ service: 'Applications' })"
        >
          <div class="heading">
      ...
      <div class="widget-def widget-application" v-if="Form.OpenLessons">
        ...
        <form
          class="vertical padding-sides"
          @submit="Add({ service: 'App_OpenLessons' })"
        >
          <div class="heading">
            <span class="h2">Записаться на открытый урок</span>
            <p>
              На открытых уроках мы рассказываем о себе, наших курсах и всей
              информации, касающейся обучения. Мероприятие проходит в формате
              презентаций и ответов на вопросы по обучению.
            </p>
          </div>
          <input
            type="text"
...
export default {
  ...
  computed: {
    ...mapGetters(["Form", "Error", "AllData"]),
    validation: function() {
      if (this.form.phone.length >= 12 && this.form.name.length > 2)
        return true;
      return false;
    }
  },
  methods: {
    ...mapActions(["Add", "OpenForm", "ShowForm", "СloseForms"])
  },
  components: {
    // InputMask
  }
